package yo.example.fingerpaint;
...
public class FingerPaintActivity extends Activity implements OnTouchListener {
	...
	void save() {
		...
		SharedPreferences pref = getSharedPreferences("FingerPaintPreferences",
				MODE_PRIVATE);
		int imageNumber = pref.getInt("imageNumber", 1);
		File file = null;
		...
		if (externalMediaChecker()) {
			...
			String path = Environment.getExternalStorageDirectory()
					+ "/mypain/";
			Log.d(ACTIVITY_SERVICE, "save() path = " + path);
	...
	private boolean writeImage(File file) {
		...
		Log.d(ACTIVITY_SERVICE, "writeImage(File file) 正常終了");
		return true;
	}
